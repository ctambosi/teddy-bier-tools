...
const navGroups = [
    {
        ...
        label: 'Correções',
        links: [
            { label: 'Densímetro',           href: route('correcao.densimetro') },
            { label: 'Refratômetro',          href: route('correcao.refratometro') },
            { label: 'Pressão × Temperatura', href: route('correcao.pressao-temperatura') },
        ],
    },
    {
        name: 'conversoes',
        label: 'Conversões',
        links: [
            { label: 'Densidade',   href: route('conversao.densidade') },
            { label: 'Temperatura', href: route('conversao.temperatura') },
            { label: 'Cor',         href: route('conversao.cor') },
            { label: 'Pressão',     href: route('conversao.pressao') },
        ],
    },
    {
        name: 'calculos',
        label: 'Cálculos',
        links: [
            { label: 'Graduação Alcoólica (ABV)', href: route('calculo.abv') },
            { label: 'Extração a Frio',           href: route('calculo.extracao-frio') },
            { label: 'Volume de Mosto',           href: route('calculo.volume-mosto') },
            { label: 'Priming',                   href: route('calculo.priming') },
            { label: 'Levedura por Peso',         href: route('calculo.levedura') },
            { label: 'Motorizar Moedor',          href: route('calculo.motor') },
            { label: 'Diluição de Álcool',        href: route('calculo.diluicao-alcoolica') },
            { label: 'Adição de Álcool',          href: route('calculo.adicao-alcoolica') },
            { label: 'Percentual de Açúcar',      href: route('calculo.percentual-acucar') },
        ],
    },
]
